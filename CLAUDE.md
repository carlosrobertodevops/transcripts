...
4. **Worker Bun** em `src/workers/loop.ts` — chama `POST /api/jobs/run` com header `x-internal-key` a cada 15s. Endpoint roda `runPendingJobs(5)` em `src/server/services/jobs.ts`.
...
# Docker (4 serviços: db, app, worker, transcriber)
...
TRANSCRIBER_URL=http://transcriber:8000   # quando provider=local
STORAGE_DIR=./uploads
